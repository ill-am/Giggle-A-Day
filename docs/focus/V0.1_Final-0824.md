...
- [x] Server test suite green (server): 24 files / 35 tests passed — Verification: commit `d374d16` and local Vitest run (24 files, 35 tests passed). Recent full run logged:

```
Test Files  24 passed (24)
    Tests  35 passed (35)
  Start at  14:44:05
  Duration  24.82s (transform 327ms, setup 0ms, collect 3.93s, tests 14.70s, environment 8ms, prepare 2.47s)
```
...
Total remaining estimate: ~14 hours (updated 2025-08-28)

Rationale: several high-effort items in sections 1 and 2 were completed and verified (image generation pipeline, jobs DB, worker, rasterization and validation helpers, and related tests). The remaining open items and conservative estimates are listed below and sum to approximately 14 hours.

- PDF Quality & Export Robustness: 3h
