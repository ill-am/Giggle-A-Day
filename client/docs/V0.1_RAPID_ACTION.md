...
```Selecting poem titles from poems in the public domain: Create a Poetry eBook evoking the summer season and activities.```
...
- 1A — typed-prompt focus textbox (no model call):

  - What: Replace the modal with a small, visible textbox placed directly under the `AetherPress` title (the "focus medium"). When users click `Generate` the app must NOT call the model; instead it will copy the exact prompt text into the textbox and immediately generate a small local preview from that typed prompt (client-side, no network). This textbox is the primary diagnostic medium for quickly verifying that the UI → store → preview injection chain is working.
  - Location / Files: add the textbox in the app header area (near `AetherPress`) and wire `client/src/components/PromptInput.svelte` so `Generate` writes the typed prompt into the textbox and sets `contentStore`/`previewStore` using the existing `buildLocalPreviewHtml` helper.
  - Data & markers: textbox should have `id="debug-focus-medium"` and `data-debug="focus-medium"`. When writing the local preview, set DOM markers on the preview container: `data-preview-source="local"` and `data-preview-local="<timestamp>"` so automated checks can detect the event.
  - Acceptance: clicking Generate with a non-empty prompt places the typed prompt into `#debug-focus-medium`, sets `contentStore` to a small `{title, body}` object and `previewStore` to the locally-built HTML; preview container gets `data-preview-source="local"` and a fresh timestamp; no network requests are made.
  - Est.: 0.15–0.5h (very small change)

  - Rationale: a visible on-page textbox is the fastest, lowest-risk medium to prove the end-to-end client path (UI -> stores -> preview injection) without worrying about modal flow, iframe sandboxing, or console flags. If the textbox receives the typed prompt and the preview still doesn't render, the failure is strongly localized to `PreviewWindow.svelte` or CSS/layout.

  - Quick verification (manual):
    1. Click Generate with a non-empty prompt.
    2. Confirm `document.getElementById('debug-focus-medium').value` equals the typed prompt.
    3. Confirm `document.querySelector('.preview-container').dataset.previewLocal` updated after the action.
    4. Confirm `document.querySelector('#preview-content').innerHTML` contains the expected local HTML.
